...
//        this.switchToEncVideo = false;
    ...
    public void setColLevel(int cl) {
        this.colLev = cl;
    }
...
/*        if (this.switchToEncVideo) {
    ...
    public void changeFilter(Image.Filter newFl, Boolean reloadModImg) {
        ...
        if (this.state == State.RUN)
            ...
            else if (this.filterStatus == Image.Filter.COLOR_QUANT)
                imgArray.get(idn).filter(filterStatus, this.colLev);      
            else
    ...
    public void writeImages(String format, File outDir) {
        this.imgArray.stream().forEach((img) -> { 
            img.writeImages(format, outDir);
        });
    }
